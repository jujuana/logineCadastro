//criar classes para mapear todas as validações existentes
class Validator{
    constructor(){
        this.validations = [
            'data-required',
    ...
    passwordvalidate(input){
        ...
        for (let i=0; charArr.length > i; i++){
            if(charArr[i] === charArr[i].toUppercase() && NaN(parseInt(charArr[i]))){
                uppercases;
        ...
        if(uppercases === 0 || numbers === 0){
            let errorMessage = `A senha precisa de pelo menos um caracter maiusculo e um numero.`;
    ...
    //verificar se o input e e requirido (exigido)
    required(input){
        let  inputValue = input.value;

        if(inputValue === ''){
            let errorMessage = 'Esse campo e de preenchimento obrigatório.';
